...
from django.utils.translation import gettext as _  # gettext vc gettext_lazy   
...
class RecipeListViewBase(ListView):
    ...
    def get_queryset(self, *args, **kwargs):
        ...
        qs = qs.select_related('author', 'category')  # quando tem FK 
        qs = qs.prefetch_related('tags', 'author__profile')  # que não tem FK
        return qs
...
class RecipeListViewCategory(RecipeListViewBase):
    ...
    def get_context_data(self, *args, **kwargs):
        ctx = super().get_context_data(*args, **kwargs)
        category_translated = _('Category')

        ctx.update({
            'title': f'{ctx.get("recipes")[0].category.name} - '
            f'{category_translated} | '
        })
